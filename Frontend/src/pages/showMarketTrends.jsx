...
export default function MarketTrendsDashboard() {
  ...
  useEffect(() => {
    async function fetchTrends() {
      try {
        const res = await fetch("http://127.0.0.1:8000/show-market-trends");
              ...
              <tbody>
                {trends.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="p-3">₹{item.avgPricePerSqft.toLocaleString()}</td>
                    <td className="p-3">₹{item.avgRentPerSqft}</td>
                    <td className="p-3 text-green-600 font-medium">{item.growthRate}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
