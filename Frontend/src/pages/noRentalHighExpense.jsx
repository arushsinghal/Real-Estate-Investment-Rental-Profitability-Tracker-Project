...
import { PieChart, AlertTriangle, Home, IndianRupee } from "lucide-react";
import '../App.css';
import '../index.css';
...
export default function NoRentalHighExpenseDashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setIsLoading(true);
        const response = await fetch("http://127.0.0.1:8000/no-rental-high-expense");
  ...
  const avgExpense = totalProperties ? (totalExpense / totalProperties).toFixed(2) : 0;
  ...
  const sortedData = [...data].sort((a, b) => b.totalExpense - a.totalExpense);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-red-100">
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 bg-white p-6 rounded-lg shadow-md border-t-4 border-red-600">
          <div>
            <h1 className="text-3xl font-bold text-red-700 flex items-center">
              ...
              No Rental, High Expense Properties
            </h1>
            <p className="text-gray-500 mt-1">Properties incurring costs without rental income</p>
          </div>
        ...
        <div className="mt-8 text-center text-gray-500 text-sm p-4 bg-white rounded-lg shadow-md">
          <p>Made with ❤️ | High Expense, No Rental Dashboard</p>
        </div>
      </div>
    </div>
  );
