...
import { TrendingUp, PieChart, Home, CalendarDays, IndianRupee } from "lucide-react";
import '../App.css';
import '../index.css';
    ...
    return (
      <div className="bg-white p-4 border border-gray-200 rounded-md shadow-lg">
        ...
        <p className="text-indigo-600 font-medium flex items-center">
          ...
          Projected Value: ₹{payload[0].value.toLocaleString(undefined, { maximumFractionDigits: 2 })}
        </p>
      </div>
    );
  }
...
export default function ProjectedValueDashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setIsLoading(true);
        const response = await fetch("http://127.0.0.1:8000/projected-value/");
  ...
  const avgProjectedValue = totalProperties
    ? (data.reduce((sum, item) => sum + item.projectedValueIn5Years, 0) / totalProperties).toFixed(2)
    : 0;
  ...
  const sortedData = [...data].sort((a, b) => b.projectedValueIn5years - a.projectedValueIn5Years);
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 bg-white p-6 rounded-lg shadow-md border-t-4 border-indigo-600">
          <div>
            <h1 className="text-3xl font-bold text-indigo-700 flex items-center">
              ...
              Projected Property Values (5 Years)
            </h1>
            <p className="text-gray-500 mt-1">Estimate based on current value & growth rate</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow-md border-t-4 border-indigo-600 hover:shadow-lg transition-shadow">
            <div className="flex items-center">
              ...
              <div>
                <p className="text-sm text-gray-500 uppercase tracking-wider">Average Projected Value</p>
                <p className="text-2xl font-bold text-gray-800">₹{Number(avgProjectedValue).toLocaleString()}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            ...
            <div className="p-4">
              <ResponsiveContainer width="100%" height={360}>
                <BarChart data={sortedData} margin={{ top: 20, right: 30, left: 20, bottom: 60 }} barSize={40}>
                  ...
                  <Bar 
                    dataKey="projectedValueIn5Years" 
          ...
          <div className="bg-white rounded-lg shadow-md overflow-hidden order-1 lg:order-2">
            <div className="bg-gradient-to-r from-indigo-600 to-indigo-500 text-white p-4 font-semibold flex items-center">
              <PieChart size={20} className="mr-2" />
              5-Year Projected Value Details
            </div>
              ...
              <table className="w-full text-left">
                <thead className="bg-gray-50 sticky top-0">
                  <tr>
                    ...
                    <th className="p-3 text-gray-600 font-semibold text-right">Current Value</th>
                    <th className="p-3 text-gray-600 font-semibold text-right">Growth Rate</th>
                    <th className="p-3 text-gray-600 font-semibold text-right">Projected Value</th>
                  </tr>
                </thead>
                <tbody>
                  {sortedData.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="p-3 text-right">₹{item.currentValue.toLocaleString()}</td>
                      <td className="p-3 text-right">{item.growthRate.toFixed(2)}%</td>
                      <td className="p-3 font-medium text-indigo-700 text-right">
                        ₹{item.projectedValueIn5Years.toLocaleString(undefined, { maximumFractionDigits: 2 })}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="mt-8 text-center text-gray-500 text-sm p-4 bg-white rounded-lg shadow-md">
          <p>Made with ❤️ | 5-Year Value Projection Dashboard</p>
        </div>
      </div>
    </div>
  );
