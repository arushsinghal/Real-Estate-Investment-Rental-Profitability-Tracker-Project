...
import { IndianRupee, PieChart, Home, TrendingUp } from "lucide-react";
import '../App.css';
import '../index.css';
    ...
    return (
      <div className="bg-white p-4 border border-gray-200 rounded-md shadow-lg">
        ...
        <p className="text-blue-600 font-medium flex items-center">
          ...
          Annual Cash Flow: {payload[0].value.toLocaleString()}
        </p>
      </div>
    );
  }
...
export default function Cashflow() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setIsLoading(true);
        const response = await fetch("http://127.0.0.1:8000/cashflow");
  ...
  const totalCashFlow = data.reduce((sum, item) => sum + item.annualCashFlow, 0);
  const avgCashFlow = totalProperties ? (totalCashFlow / totalProperties).toFixed(2) : 0;
  
  // Get current date for dashboard timestamp
  ...
  // Sort data by cash flow (highest to lowest)
  const sortedData = [...data].sort((a, b) => b.annual_cash_flow - a.annual_cash_flow);
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 bg-white p-6 rounded-lg shadow-md border-t-4 border-blue-600">
          <div>
            ...
            <p className="text-gray-500 mt-1">Property investment performance at a glance</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow-md border-t-4 border-indigo-500 hover:shadow-lg transition-shadow">
            <div className="flex items-center">
              <div className="bg-indigo-100 p-3 rounded-full mr-4">
                <IndianRupee size={24} className="text-indigo-600" />
              </div>
              <div>
                <p className="text-sm text-gray-500 uppercase tracking-wider">Total Cash Flow</p>
                <p className="text-2xl font-bold text-gray-800">₹{totalCashFlow.toLocaleString()}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-md border-t-4 border-indigo-600 hover:shadow-lg transition-shadow">
            <div className="flex items-center">
              ...
              <div>
                <p className="text-sm text-gray-500 uppercase tracking-wider">Average Cash Flow</p>
            ...
            <div className="p-4">
              ...
              {data.length > 10 && (
                <p className="text-center text-xs text-gray-500 mt-2">
                  * Showing top 10 properties by cash flow
                </p>
              )}
            </div>
          </div>
                ...
                <tbody>
                  {sortedData.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="p-3 font-medium text-blue-700 text-right">
                        ₹{item.annualCashFlow.toLocaleString()}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="mt-8 text-center text-gray-500 text-sm p-4 bg-white rounded-lg shadow-md">
          <p>Made with ❤️ | Real Estate Cash Flow Dashboard</p>
        </div>
      </div>
    </div>
  );
