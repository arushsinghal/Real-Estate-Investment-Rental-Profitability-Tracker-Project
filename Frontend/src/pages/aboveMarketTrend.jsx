      ...
      try {
        ...
        if (Array.isArray(json)) {
          const enriched = json.map(item => ({
            ...item,
            delta: item.monthlyRent - item.marketEstimatedRent,
          }));
          setData(enriched);
        } else {
                ...
                <tbody>
                  {sortedData.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="p-3 text-right">₹{item.monthlyRent.toLocaleString()}</td>
                      <td className="p-3 text-right">₹{item.marketEstimatedRent.toLocaleString()}</td>
